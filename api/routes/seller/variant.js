...
router.post('/add-variant/:id', [checkAuth, imgUpload], async (req, res, next) => {
    ...
    try {
        ...
        var variantFile = req.files.images
        var imageArr = [];

        (async function () {
            for await (const file of variantFile) {

                const imageRef = storage.child("/variants/" + file.originalname);
