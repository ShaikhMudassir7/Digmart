  ...
  Seller.findById(req.params.id, (err, doc) => {
    if (!err) {
      SellerGall.find({ sellerID: id })
        ...
        // .exec()
        .then((docs) => {
          res.render("./seller/gallery/gallery", {
            galleryData: docs,
            id: id,
            sellerData: doc,
            sellerID: id,
            pFname: req.session.pFname,
            pLname: req.session.pLname,
          });
          // .exec()
        })
        .catch((err) => {
...
router.get("/add-gallery/:id", checkAuth, (req, res) => {
  ...
  Seller.findById(req.params.id, (err, doc) => {
    if (!err) {
      SellerGall.find({ sellerID: sellerID })
        .select()
        .exec()
        .then((docs) => {
          res.render("./seller/gallery/add-gallery", {
            sellerData: doc,
            id: sellerID,
            galleryData: docs,
    ...
    if (rawSS) {
      for (let i = 0; i < rawSS.length; i++) {
        const imageRef = storage.child(
          "/gallery/" +
            (rawSS[i].fieldname + "-" + Date.now() + rawSS[i].originalname)
        );
...
router.get("/edit-gallery/(:id)/(:galleryID)", checkAuth, (req, res) => {
  ...
  Seller.findById(id, (err, element) => {
    if (!err) {
      SellerGall.findById(galleryID, (err, doc) => {
        if (!err) {
          SellerGall.find({ sellerID: id })
            .select()
            .exec()
            .then((docs) => {
              res.render("./seller/gallery/edit-gallery", {
                images: allImages,
                galleryData: doc,
                sellerID: req.session.sellerID,
                pFname: req.session.pFname,
